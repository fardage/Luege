...
/// View for displaying TV shows from a library folder with poster grid
struct TVShowLibraryFolderView: View {
    ...
    var body: some View {
        Group {
            if files.isEmpty {
                ...
                // Still fetching metadata, show empty space (overlay handles progress)
                Color.clear
            }
        }
        .id(metadataService.tvMetadataVersion)  // Force view rebuild when TV metadata updates
    ...
    /// Get unique TV shows from all episode metadata, sorted by name
    /// Note: Access tvMetadataVersion to ensure SwiftUI tracks cache updates
    private var uniqueShows: [TVShowMetadata] {
        // Reference tvMetadataVersion to create SwiftUI dependency on cache updates
    ...
    /// Get number of episodes available for a TV show
    ...
    /// Get file mapping for a TV show (episode ID -> LibraryFile)
    ...
    private func fetchMetadataIfNeeded() async {
        ...
        // Filter files that don't have TV episode metadata yet
